...
export default function OrderPartsSelector({
  ...
  async function loadData() {
    ...
    const { data: items, error: inventoryError } =
      await supabase
  ...
  async function addPart() {
    ...
    const price = Number(
      unitPrice || selectedItem.sale_price || 0
    )
    ...
    const available =
      Number(selectedItem.quantity_on_hand || 0) -
      Number(selectedItem.quantity_reserved || 0)
  ...
  async function usePart(part) {
    if (
      !confirm(
        'Confirmar uso desta peça? O estoque físico será baixado.'
      )
    ) {
      return
    }
  ...
  async function releasePart(part) {
    if (
      !confirm(
        'Cancelar a reserva desta peça e devolver ao estoque disponível?'
      )
    ) {
      return
    }
    ...
    <div className="space-y-4">
      ...
      <div className="border rounded-2xl p-4 space-y-4">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Reserve peças do estoque e controle o custo do serviço.
          </p>
        </div>
      ...
      <div className="space-y-3">
        {displayedParts.length === 0 ? (
          <div className="border rounded-2xl p-4 text-sm text-slate-500">
            Nenhuma peça vinculada a esta OS.
          </div>
        ) : (
          displayedParts.map((part, index) => (
            <div
              key={part.id || `${part.inventory_item_id}-${index}`}
            ...
            >
              <div className="flex justify-between gap-3">
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    {part.quantity} x {money(part.unit_price)}
                  </p>
                </div>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {part.status === 'reserved' && (
                    <>
                      ...
                      <button
                        type="button"
                        onClick={() => releasePart(part)}
                        disabled={busy}
                        className="px-3 py-2 rounded-xl border border-red-200 text-red-600 text-sm font-semibold disabled:opacity-50"
                      >
                        Cancelar reserva
                      </button>
                    </>
                  ...
                  {part.status === 'used' && (
                    <span className="text-xs font-semibold px-3 py-2 rounded-full bg-emerald-50 text-emerald-700">
                      Peça utilizada
                    </span>
                  )}

                  {part.status === 'cancelled' && (
                    <span className="text-xs font-semibold px-3 py-2 rounded-full bg-slate-100 text-slate-500">
                      Reserva cancelada
                    </span>
                  )}
                </div>
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-2xl p-4 flex justify-between items-center gap-3">
        <div>
          ...
          <p className="text-xs text-blue-600">
            Valor de venda das peças vinculadas
          </p>
        </div>
